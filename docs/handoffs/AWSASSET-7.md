...
**Implementation:** The React protected demo uses backend identity/group results, signed-out gating and 401/403 handling. Local-only origin/port is configurable. AWS URLs, issuer, client, pool, scope and redirect settings come from public configuration. frontend/team-config.js and team-auth.js define the AWSASSET-6 integration seam; login, logout, and password reset remain assigned to AWSASSET-6.
